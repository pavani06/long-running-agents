...
| Auto-agendamento (alarm record) | O agente consegue agendar a própria próxima retomada gravando alarm record durável com `wake_at`, `wake_reason` e `state_ref`, conforme [[docs/canonical/alarm-clock-agent-lifecycle\|Alarm-Clock Agent Lifecycle]]. | Existe evidência verificável e atualizada. | Retomada depende só de gatilho externo ou poller central; o agente nunca escolhe quando voltar. | Registre link para artefato, owner e data. |
| Wake trigger durável | O alarme vive fora do processo do agente; o sleep sobrevive a restart de VM e o wake injeta `wake_reason` + desserializa `state_ref` para continuar exatamente de onde pausou. | Existe evidência verificável e atualizada. | Alarme perdido é silêncio; não há teste de alarme faltante. | Registre link para artefato, owner e data. |
