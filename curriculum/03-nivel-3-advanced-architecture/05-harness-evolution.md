...
tags: ["curriculo-conteudo", "nivel-3", "arquitetura-avancada", "harness", "arquitetura", "shadow-review", "canary-test", "stack-tooling", "decision-discipline", "evals"]
...
### O Teto do Meta-Loop: Quem Melhora o Próprio Agente?

O GC Day e o ciclo trimestral compartilham um teto silencioso: ambos melhoram **artefatos externos ao agente** — lint rules, rubricas, casos de regressão, componentes do harness. O feedback acumulado existe, mas disperso por docs, eval sets e tickets; nenhum agente raciocina sobre o conjunto. A arquitetura-alvo que fecha essa lacuna tem nome: **Self-Iterating Agent Loop** — todas as partes do produto alimentando uma fundação de dados unificada sobre a qual os agentes raciocinam, com orquestração e execução sobre o mesmo plano, observação dos resultados de volta nele, e agentes construindo iterações melhores de si mesmos (prompts, skills, tools, data models derivados do próprio histórico, não de especificação humana ex-nihilo) ([[docs/canonical/self-iterating-agent-loop|Self-Iterating Agent Loop]]).

Duas ressalvas de engenharia antes de tratar isso como receita. Primeiro, o loop é uma arquitetura-alvo declarada, não uma implementação pronta: ele é tão forte quanto o elo mais fraco (observação, feedback ou dados) e depende da migração para a fundação de dados unificada — o maior pré-requisito, porque sem substrato único o loop fecha sobre silos ([[docs/canonical/agent-first-data-foundation|Agent-First Data Foundation]]). Segundo, auto-iteração não-gateada **amplifica** drift: cada iteração herda e agrava os vieses do feedback que a produziu. O gate é o [[docs/canonical/confidence-gated-continual-learning|Confidence-Gated Continual Learning]] já vinculado a esta lição: melhoria proposta pelo agente só é incorporada automaticamente acima de threshold de confiança; abaixo dele, revisão humana.
