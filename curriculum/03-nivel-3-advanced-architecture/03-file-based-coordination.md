...
**Skills and CLI as Native Agent Data Access** — O file system é o substrato universal; a **CLI dedicada** é a forma concreta desse substrato para acesso a dados: os mesmos comandos que um humano usaria, sem UI obrigatória no caminho, pareada com skills agent-oriented que encapsulam o know-how de operação da plataforma (como consultar, como modelar, como não atirar no próprio pé). A delegação é goal-level — um statement de objetivo ("quero este data model"), não um script passo-a-passo — e a execução é longa por design: 1-2 horas com budget, retomada e monitoramento, cujo output é um **novo data model**, não um relatório.

- **Documento canônico:** [[docs/canonical/skills-cli-native-agent-data-access]]
- **Relevância para File-Based Coordination:** Este módulo ensina o file system como *communication bus* entre agentes; o padrão estende a mesma tese para o acesso a dados — quando a plataforma de dados é consumível por CLI + arquivos + skills, o agente vira cliente first-class sem adaptador humano no meio (dashboards, exports, tickets). A ponte curricular completa: file-system materialization → delegação goal-level com estado serializável → deploy do artefato produzido no shadow target separado ([[docs/canonical/shadow-builds-separated-compute|Shadow Builds on Separated Compute]]).
