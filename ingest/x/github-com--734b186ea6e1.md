...
key: "734b186ea6e1"
status: "ok"
final_url: "https://github.com/jordan-gibbs/hyperresearch"
method: "trafilatura"
content_hash: "19ec756d7347bd78c099c485f3305d3cd64a2161"
text_len: 28356
fetched: "2026-09-13"
---
...
- Schema integrity. tier ,content_type , andtype are SQLite CHECK-constrained vocabularies; corrupted frontmatter cannot poison the index
...
- Fetched text is data, never instructions. Web-fetched bodies are served inside an <untrusted-source> fence on bothnote show andsearch , so a page telling the agent to ignore its instructions is read as content
...
- parallel — Parallel's Search MCP endpoint, which needs no account or key. Search only — bulk fetch waves degrade to per-URL, so it is a good search provider rather than a replacement for the crawl4ai fetch path. Every request from one process carries a random session ID that Parallel uses for correlation and rate limiting on its side.pip install "hyperresearch[parallel]" .
- serply — Serply's Google search API, with page fetch through the same key (API docs). Search results are fetched to full page text, falling back to the snippet when a page cannot be fetched. Needs an API key; no extra install.
...
- OpenAlex — ~250M works across every discipline, including books, book chapters and theses. The right default outside STEM, where the incumbent tools are weakest.
- Crossref — the DOI registry itself. Authoritative metadata for ~160M registered works, including registrations too new for anything else to have indexed.
- CORE — the largest open-access full-text aggregator. Hosts the text rather than linking to it, so it is also a full-text resolver (below). Needs a key: CORE_API_KEY .
- DOAB — peer-reviewed open-access scholarly books and chapters. The only source here that finds the book rather than a review of it, which matters because in the humanities the book, not the article, is the unit of publication.
- RePEc — listed so the gap is visible rather than silent. Their API has no search function; hpr scholar sources says so and points at OpenAlex and Crossref for the DOI-bearing series.
...
- ClinicalTrials.gov — registered studies including the ones that never produced a paper. No key.
- SEC EDGAR — full-text search over filings. The SEC rejects any request without a contact address in the User-Agent, so set HYPERRESEARCH_CONTACT_EMAIL .
- FRED — Federal Reserve economic series. Needs a key: FRED_API_KEY , which is never written to the cache.
...
- A banner at the top of the note body, naming the URL the text actually came from and which version it is.
- oa_url /oa_source /oa_version /oa_license /oa_recovery_kind in the note's frontmatter.
- An oa block inhpr note show <id> --json , carryingbody_is_not_from_source: true .
- A line in the output of hpr fetch andhpr fetch-batch .
