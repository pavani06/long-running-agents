...
key: "ae0a2a4ad026"
status: "ok"
final_url: "https://platform.claude.com/docs/en/cli-sdks-libraries/cli/apply"
method: "trafilatura"
content_hash: "5e7b95ad864b8c424dded5c8ad04d72c577d2930"
text_len: 10739
fetched: "2026-09-13"
---

Manage resources as code with ant apply
Declare agents, environments, skills, memory stores, and deployments as files in your repository and keep the API's resources in sync with them using ant apply.
...
To install and authenticate the CLI, see the CLI quickstart. ant apply requires CLI version 1.30.0 or later.
...
The frontmatter holds the agent's configuration (the fields from Define your agent) and the body is its system prompt. ant apply infers that the file is an agent from its path, here the agents/ directory.
...
Commit it with your files. It's how the next run, on your machine or in CI, finds these resources instead of creating them again, and it's where you read an agent's ID to start a session. The two hashes fingerprint what was last sent and what the API returned. That's how a later run notices an edited file, or a resource changed outside these files.
...
- An environment is a YAML file in environments/ .
- A memory store is a YAML file in memory_stores/ .
- A deployment is a Markdown file in deployments/ : the frontmatter is the request body and the prose becomes the message that starts each session.
- A skill is a directory with a SKILL.md at its root, conventionally underskills/ , uploaded as one bundle.
...
To point at a resource these files don't manage, write its ID (agent_..., skill_...) instead. Anything else, such as {type: anthropic, skill_id: xlsx}, is sent to the API as written. A skill reference can also be a GitHub URL of the form https://github.com/<owner>/<repo>/tree/<branch>/<dir>, for example a directory of Anthropic's open-source skills repository: ant apply downloads and uploads that directory, pinned to the resolved commit until you run with --upgrade (set GITHUB_TOKEN for a private repository).
...
- The directory the file is directly in: agents/ ,environments/ ,memory_stores/ , ordeployments/ .
- A file name that starts with the kind, such as environment_staging.md .
...
- Run ant apply --yes . on your default branch after merge, naming the project directory. A bareant apply --yes reconciles only files the lockfile already tracks and skips a newly added one.
...
- Authenticate with Workload Identity Federation rather than a stored API key, as an identity that reaches the organization and workspace recorded in claude-lock.json .ant apply refuses credentials that resolve to any other organization or workspace.
For a complete GitHub Actions workflow, see the CI example in the CLI README.
Flags
| Flag | Effect | 
|---|---|
