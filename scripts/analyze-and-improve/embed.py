...
The repo's retrieval memory reuses the same embedding technique as the
connections layer (`scripts/youtube-connections/embed.py`). Kept as its own
module so the pure control-plane/index logic stays free of network I/O and
unit-testable; this client is exercised only by the pipeline runner, never by
the tests.
"""
