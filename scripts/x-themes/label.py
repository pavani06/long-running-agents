"""Deterministic theme labels from the tags/key_points of a community's members."""
...
def auto_label(members: list[str], extracts: dict[str, ExtractMeta],
               *, n_tags: int = 3, n_points: int = 2) -> dict:
    """Return {'top_tags': [...], 'top_points': [...], 'label_auto': str}."""
    tags = [t for t, _ in _counter(members, extracts, "tags").most_common(n_tags)]
    points = [c for c, _ in _counter(members, extracts, "key_points").most_common(n_points)]
    label = ", ".join(tags) if tags else "tema"
    return {"top_tags": tags, "top_points": points, "label_auto": label}
