#!/usr/bin/env python3
"""Digest generator for the X bookmarks corpus.

Groups in-scope extracts by their theme (Fase 4), synthesizes each theme with
GLM (concrete argument + non-obvious + project-anchored actions), ranks a
"read in this order" list, surfaces the thin bucket, and writes a Markdown
digest. Derives everything from committed artifacts.

Modes:
  bootstrap   the whole acervo, organized by theme (one-time)
  daily       only bookmarks collected today (default)
...
DIGESTS_DIR = REPO_ROOT / "digests" / "x"
PROJECTS_MD = DIGESTS_DIR / "projects.md"
...
def _chips(members: list[Extract], neighbors: dict[str, list[str]],
    ...
    member_ids = {e.status_id for e in members}
    theme = members[0].theme if members else ""
    out: list[Extract] = []
    seen: set[str] = set()
    for e in members:
        for nid in neighbors.get(e.status_id, []):
            n = by_id.get(nid)
...
def run(mode: str, zai_key: str | None) -> int:
    ...
    projects = parse_projects(PROJECTS_MD.read_text(encoding="utf-8")) if PROJECTS_MD.exists() else []
