#!/usr/bin/env python3
"""Connections layer for the X bookmarks extract corpus.

Embeds every extract (topic+summary+key_points+tags) with OpenAI, builds a
...
use / theme clustering). Batch job: recomputes the whole graph each run.
...
def run(api_key: str, k: int, floor: float | None, dry_run: bool) -> int:
    ...
    ids = [e.status_id for e in extracts]
    by_id = {e.status_id: e for e in extracts}
    try:
    ...
    changed = 0
    for e in extracts:
        links = [wikilink(by_id[nid].file, by_id[nid].title) for nid, _w in nbrs.get(e.status_id, [])]
