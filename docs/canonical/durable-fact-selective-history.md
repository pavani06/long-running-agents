...
tags: ["context-engineering", "agentes-orquestracao", "context-management", "agentic-coding"]
...
| Omission path | Drop transient turns from active context once they are outside the recent window, unless exact recovery is needed through addressable memory. [[docs/canonical/addressable-memory-catalog|Addressable Memory Catalog]] defines `id`, `kind`, `location`, `preview`, `scope`, `tool`, and `path` for omitted content (`fetch` deprecated per [[docs/canonical/addressable-memory-catalog|Addressable Memory Catalog]]:38-43) ([[docs/canonical/addressable-memory-catalog|Addressable Memory Catalog]]:28-43). |
