...
tags: ["context-engineering", "agentes-orquestracao", "agent-loop", "context-management", "agentic-coding"]
...
6. **Preserve exact recovery.** Store or reference the source span through [[docs/canonical/addressable-memory-catalog|Addressable Memory Catalog]] handles. The catalog provides `id`, `kind`, `location`, `preview`, `scope`, `tool`, and `path` fields (`fetch` deprecated per [[docs/canonical/addressable-memory-catalog|Addressable Memory Catalog]]:38-43) for exact omitted-content recovery ([[docs/canonical/addressable-memory-catalog|Addressable Memory Catalog]]:28-43).
