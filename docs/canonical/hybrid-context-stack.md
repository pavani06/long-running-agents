...
tags: ["context-engineering", "agentes-orquestracao", "context-management", "agentic-coding", "agent-loop"]
...
| 7 | Addressable omitted-memory catalog | Include compact `id`, `kind`, `location`, `preview`, `scope`, `tool`, and `path` metadata (no `fetch` — see [[docs/canonical/addressable-memory-catalog|Addressable Memory Catalog]]:38-43) | Shrink previews, not stable IDs or `tool`+`path` handles | Catalog fields make omitted content discoverable and exactly recoverable ([[docs/canonical/addressable-memory-catalog|Addressable Memory Catalog]]:30-43). |
