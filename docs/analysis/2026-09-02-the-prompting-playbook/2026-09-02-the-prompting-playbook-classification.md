...
# Classification: The Prompting Playbook Patterns vs. long-running-agents

Consolidated from two classification batches (12 patterns, max 8 per agent per harness rule). Precedence per `docs/system-of-record.md:14-21`: decisions/ > canonical/ > evidence/ > analysis/ > curriculum/ > READMEs. Every classification cites file:line; `Missing` confirms NOT_FOUND with locations searched. Batch evidence preserved in `classification-batch-1.md` and `classification-batch-2.md`.
